...
data = []
for cur_id, a in manga_list.items():
    data.append(a.score)
a = np.array(data)
...
with open(os.path.join(__location__, 'manga_data.json')) as data_file:
...
for d in sigma_data['nodes']:
    ...
    cur_url = '<a href="https://myanimelist.net/manga/' + str(cur_id) + '">' + a.name + '</a>'
...
with open(os.path.join(__location__, 'manga_data.json'), 'w') as outfile:  
